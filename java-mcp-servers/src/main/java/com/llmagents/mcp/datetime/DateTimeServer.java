...
/**
 * DateTime MCP Server main class.
 * Provides getDateTime tool via MCP protocol.
 */
@Command(name = "datetime-mcp-server", mixinStandardHelpOptions = true,
         version = "1.0.0", description = "DateTime MCP Server")
public class DateTimeServer implements Callable<Integer> {
    private static final Logger logger = LoggerFactory.getLogger(DateTimeServer.class);
    ...
    public static void main(String[] args) {
        int exitCode = new CommandLine(new DateTimeServer()).execute(args);
    ...
    public Integer call() throws Exception {
        ...
        ServerConfig config = ServerConfig.create(
            8082,  // default HTTP port
            8444,  // default HTTPS port
            "DATETIME_MCP_PORT",
            "DATETIME_MCP_TLS_PORT",
            tlsFlag,
            verbose
        );
        ...
        MCPServlet mcpServlet = new MCPServlet(
            "datetime-mcp",
            dateTimeTool::handleToolCall,
            List.of(DateTimeTool.getToolDefinition())
        );
        ...
        try {
            server.start();
            logger.info("DateTime MCP Server started successfully on {}://localhost:{}",
                config.effectiveProtocol(), config.effectivePort());
            server.join();
            return 0;
        } catch (Exception e) {
            logger.error("Failed to start DateTime MCP Server", e);
            return 1;
        }
    }
}
